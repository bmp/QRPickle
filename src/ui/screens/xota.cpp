...
namespace ui {
    ...
    static void set_tab(XotaTab t) {
        active_tab = t;
        if (t == TAB_POTA) {
            // FIXED: Weather screen tab styling (Active)
            ...
            // FIXED: Weather screen tab styling (Inactive)
    ...
    static void update_ui(lv_timer_t* t) {
        ...
        bool fetching = (active_tab == TAB_POTA) ? services::PotaManager::is_fetching() : services::SotaManager::is_fetching();
    ...
    void draw_xota_page(lv_obj_t* parent) {
        ...
        // --- Styled Split Tab Bar (Matches Weather Screen) ---
        ...
        lv_obj_set_style_bg_color(btn_tab_pota, theme_color(COLOR_BG_PANEL), 0); // Base color
        ...
        lv_obj_set_style_text_font(lbl_tab_pota, &font_atkinson_14, 0); // Weather font
        ...
        lv_obj_set_style_bg_color(btn_tab_sota, theme_color(COLOR_BG_PANEL), 0); // Base color
        ...
        lv_obj_set_style_text_font(lbl_tab_sota, &font_atkinson_14, 0); // Weather font
        lv_obj_center(lbl_tab_sota);

        // --- Data Header Row (Matches DX Spots Screen) ---
        lv_obj_t* head = lv_obj_create(scr);
        lv_obj_set_size(head, 320, 26);
        lv_obj_align(head, LV_ALIGN_TOP_MID, 0, 24);
        lv_obj_set_style_bg_color(head, theme_color(COLOR_BG_PANEL), 0); // DX cluster style
        ...
        // Styled click boxes for the headers
        ...
        lv_obj_set_size(list_container, 320, 166); // Height correctly calculated: 240 - 24(tab) - 26(hdr) - 24(foot)
        ...
        lv_obj_add_event_cb(scr, [](lv_event_t*){
            ...
            scr = list_container = lbl_comment = btn_freq = btn_mode = btn_qrp = lbl_tab_pota = lbl_tab_sota = lbl_hdr_ref = btn_tab_pota = btn_tab_sota = nullptr;
